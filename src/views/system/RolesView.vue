...
<template>
  <section class="page-content page-stack list-page-layout">
    ...
    <AppCard as="section" padding="none" fill-height class="data-table-card" aria-label="角色列表"><AppTableToolbar><AppButton leading-icon="plus" @click="openCreateDrawer">创建角色</AppButton><template #actions><AppTableOperationBar show-refresh :refresh-disabled="isLoading" refresh-label="刷新角色列表" @refresh="refreshRoles" /></template></AppTableToolbar><AppDataTable :rows="roles" :columns="roleColumns" row-key="id" :loading="isLoading" :error-message="errorMessage" fill-height action-label="操作" empty-title="没有找到匹配角色" empty-description="调整搜索条件后再试一次。" empty-icon="shield" @retry="refreshRoles"><template #cell-name="{ row }"><strong class="role-name">{{ row.name }}</strong><span class="role-code">{{ row.code }}</span></template><template #cell-description="{ row }"><span class="description-cell">{{ row.description }}</span></template><template #cell-permissions="{ row }"><span class="permission-summary">{{ row.permissions.join("、") }}</span></template><template #cell-memberCount="{ row }">{{ row.memberCount }} 人</template><template #actions="{ row }"><AppTableActions><AppIconButton icon="edit" label="配置角色权限" size="small" @click="openEditDrawer(row)" /><AppIconButton icon="trash" label="删除角色" size="small" variant="danger" @click="deleteTarget = row" /></AppTableActions></template></AppDataTable><AppPagination v-if="!isLoading && !errorMessage && roles.length" :page="page" :page-size="pageSize" :total="total" :page-size-options="[10, 20, 30, 50, 100]" @update:page="handlePageChange" @update:page-size="handlePageSizeChange" /></AppCard>
...
<style scoped>
.role-name, .role-code { display: block; }.role-name { color: var(--aps-ink); font-size: var(--aps-text-base); font-weight: var(--aps-font-weight-strong); }.role-code { margin-top: 3px; color: var(--aps-faint); font-size: var(--aps-text-xs); }.description-cell { display: inline-block; min-width: 190px; max-width: 300px; color: var(--aps-muted); line-height: 1.55; white-space: normal; }.permission-summary { display: inline-block; max-width: 180px; overflow: hidden; color: var(--aps-muted); text-overflow: ellipsis; white-space: nowrap; }.drawer-form { display: grid; gap: var(--aps-form-gap); }.role-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }.permission-section { padding-top: 3px; }.permission-section header h3, .permission-section header p { margin: 0; }.permission-section header h3 { color: var(--aps-ink); font-size: var(--aps-text-base); font-weight: var(--aps-font-weight-heading); }.permission-section header p { margin-top: 4px; color: var(--aps-muted); font-size: var(--aps-text-sm); }.permission-groups { display: grid; gap: 10px; margin-top: 12px; }.permission-groups section { padding: 16px; border: 1px solid var(--aps-line-soft); border-radius: 10px; }.permission-groups h4 { margin: 0 0 10px; color: var(--aps-ink); font-size: var(--aps-text-sm); font-weight: var(--aps-font-weight-strong); }.permission-groups :deep(.app-checkbox-group-option) { min-height: 30px; }.form-error { margin: -3px 0 0; color: var(--aps-red); font-size: var(--aps-text-sm); }@media (max-width: 680px) { .role-form-grid { grid-template-columns: 1fr; } }
</style>
