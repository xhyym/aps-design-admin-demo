...
<template>
  <section class="page-content page-stack list-page-layout">
    ...
    <AppCard as="section" padding="none" content-overflow="visible" fill-height class="data-table-card" aria-label="菜单列表"><AppTableToolbar><AppButton leading-icon="plus" @click="openCreateDrawer">新增菜单</AppButton><template #actions><AppIconButton icon="refresh" label="刷新菜单列表" :disabled="isLoading" @click="refreshMenus" /></template></AppTableToolbar><AppDataTable :rows="menus" :columns="menuColumns" row-key="id" :loading="isLoading" :error-message="errorMessage" fill-height action-label="操作" empty-title="没有找到匹配菜单" empty-description="调整搜索条件后再试一次。" empty-icon="menu" @retry="refreshMenus"><template #cell-name="{ row }"><strong class="menu-name">{{ row.name }}</strong></template><template #cell-path="{ row }"><code>{{ row.path }}</code></template><template #cell-permission="{ row }"><code>{{ row.permission }}</code></template><template #cell-status="{ row }"><AppStatusTag :tone="getStatus(row).tone" :label="getStatus(row).label" /></template><template #actions="{ row }"><div class="table-actions"><AppButton variant="text" size="small" @click="openEditDrawer(row)">编辑</AppButton><AppButton variant="text" size="small" @click="deleteTarget = row">删除</AppButton></div></template></AppDataTable><AppPagination v-if="!isLoading && !errorMessage && menus.length" :page="page" :page-size="pageSize" :total="total" :page-size-options="[10, 20, 30, 50, 100]" @update:page="handlePageChange" @update:page-size="handlePageSizeChange" /></AppCard>
...
<style scoped>
.menu-name { color: var(--aps-ink); font-size: var(--aps-text-base); font-weight: 680; }code { color: var(--aps-muted); font-family: var(--aps-font); font-size: var(--aps-text-sm); }.table-actions { display: inline-flex; align-items: center; gap: 7px; }.drawer-form { display: grid; gap: var(--aps-form-gap); }.menu-form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }.form-error { margin: -3px 0 0; color: var(--aps-red); font-size: var(--aps-text-sm); }@media (max-width: 680px) { .menu-form-grid { grid-template-columns: 1fr; } }
</style>
